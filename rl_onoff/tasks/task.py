"""Task interface for task-specific prompt templates and rewards."""
...
class Task:
    """Task class for configuring prompt templates, rewards, and formats.
    ...
    def __init__(self, config: Union[str, Path, TaskConfig, Dict[str, Any]]):
        ...
        # Build reward config with name (MathVerifyReward takes no parameters)
        reward_config = {"name": self.config.reward_type}
        self.reward = create_reward(reward_config)
    ...
    def format_query(self, question: str, add_generation_prompt: bool = True) -> str:
    ...
    def evaluate(
        ...
        # Compute scores
        scores = self.reward.compute(answers, references)
